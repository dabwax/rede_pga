<div class="inputs form large-10 medium-9 columns" ng-controller="CmsInputCtrl">
    ...
    <fieldset ng-init='input = <?php echo json_encode($input); ?>;'>
        ...
        <div class="config" ng-if="input.type == 'escala_texto'">
            ...
            <a href="javascript:;" class="btn btn-xs btn-info" style="margin-bottom: 20px; margin-top: 20px;" ng-click="input.config.options.push('')">Adicionar +</a>

            <div class="clearfix"></div>

            <div class="form-group">
                <textarea class="form-control" name="config[options][{{index}}]" ng-repeat="(index, option) in input.config.options track by $index">{{option}}</textarea>
            </div>
        </div>
    </fieldset>
